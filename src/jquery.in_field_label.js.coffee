...
jQuery ($) ->
  ...
  find_self_options_for = ($input) ->
    opts = {}
    $.each ['align', 'padding', 'opacity', 'animate_duration'], (i, name) ->
      opts[name] = $input.data(name)
    opts
    ...
    this.filter(valid_selectors).each ->
      ...
      toggle_label = (e) ->
        ...
        ($this.val().length == 0 && e && e.keyCode > 31) # still typing
          $label.animate {'opacity': 0}, options.animate_duration
        else
          $label.animate {'opacity': options.opacity}, options.animate_duration
